...
class SteelDefectTrainer:
    """
    钢铁表面缺陷分类训练器（仅支持 MobileSteelNet 模型）
    """
    ...
    def _create_model(self):
        """创建 MobileSteelNet 模型"""
        num_classes = self.config.get('num_classes', 6)
        in_channels = self.config.get('in_channels', 3)

        model = MobileSteelNet(num_classes=num_classes, in_channels=in_channels)
        print(f"创建模型: MobileSteelNet (num_classes={num_classes})")
...
def main():
    ...
    print("钢铁表面缺陷分类训练 - MobileSteelNet")
    print("=" * 50)
    print(f"数据集: {config['dataset_type']}")
    print(f"模型: MobileSteelNet")
